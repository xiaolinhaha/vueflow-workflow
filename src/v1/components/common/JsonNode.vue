<template>
  <div class="select-text" :class="isRoot ? '' : 'pl-5'">
    <!-- 对象或数组 -->
    <div v-if="isExpandable" class="space-y-0.5">
      ...
      >
        <!-- 展开/折叠图标 -->
        <span
          class="inline-flex items-center justify-center w-4 h-4 text-gray-400 transition-transform"
    ...
    <div v-else class="flex items-center gap-1 px-1">
      <!-- 占位符，保证与可折叠节点对齐 -->
      <span class="inline-flex w-4 h-4"></span>
      ...
      <span
        ...
        >{{ name }}</span
      >
      <span v-if="name !== null" class="text-gray-500">: </span>

      <!-- 值 -->
      <span
        :class="['flex-1 min-w-0 truncate', valueClass]"
